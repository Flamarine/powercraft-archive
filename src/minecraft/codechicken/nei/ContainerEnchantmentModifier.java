...
public class ContainerEnchantmentModifier extends ContainerEnchantment
{
    ...
    {
    	NBTTagList nbttaglist = ((Slot)inventorySlots.get(0)).getStack().getEnchantmentTagList();
        if(nbttaglist != null)
        {
            for(int i = 0; i < nbttaglist.tagCount(); i++)
            {
                ...
                if(ID == e)
                {
                	try
					{
                		List<?> tagList = nbttaglist.tagList;
                    	tagList.remove(i);
                    	if(tagList.size() == 0)
                    	{
                    		((Slot)inventorySlots.get(0)).getStack().setTagCompound(null);
                    	}
					}
					catch(Exception ex)
					{
