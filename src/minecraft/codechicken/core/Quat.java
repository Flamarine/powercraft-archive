...
public class Quat
{
    ...
    public void setAroundAxis(double ax, double ay, double az, double angle)
    {
        angle *= 0.5D;
        double d4 = Math.sin(angle);
        set(Math.cos(angle), ax * d4, ay * d4, az * d4);
    }

    private void set(double d, double d1, double d2, double d3)
    {
        x = d1;
        y = d2;
        z = d3;
