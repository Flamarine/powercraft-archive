...
public class If extends PostfixMathCommand implements CallbackEvaluationI {
	...
	*/
	...
	public Object evaluate(Node node, EvaluatorI pv) throws ParseException {
		...
		if (!checkNumberOfParameters(num)) throw new ParseException("If operator must have 3 or 4 arguments.");
		...
		if (condVal instanceof Boolean) {
			if (((Boolean) condVal).booleanValue()) return pv.eval(node.jjtGetChild(1));
		...
		else if (num == 3 || val < 0.0) return pv.eval(node.jjtGetChild(2));
