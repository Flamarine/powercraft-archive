...
import powercraft.weasel.jep.EvaluatorI;
import powercraft.weasel.jep.Node;
import powercraft.weasel.jep.ParseException;
import powercraft.weasel.jep.Variable;
...
public class Ele extends PostfixMathCommand implements LValueI {
	...
	public void set(EvaluatorI pv, Node node, Object value) throws ParseException {
		...
		if (!(lhsNode instanceof ASTVarNode)) throw new ParseException("Ele: lhs must be a variable");
		...
		if (!(oldVarVal instanceof Vector)) throw new ParseException("Ele: the value of the variable must be a Vector");
