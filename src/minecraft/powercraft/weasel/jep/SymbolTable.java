...
public class SymbolTable extends Hashtable<String, Variable> {
	...
	public Variable put(String key, Variable val) {
		return makeVarIfNeeded(key, val);
	}
	...
	public Variable addVariable(String name, Object val) {
		Variable var = super.get(name);
		if (var != null) throw new IllegalStateException("Variable " + name + " already exists.");
	...
	public Variable makeVarIfNeeded(String name, Object val) {
		Variable var = super.get(name);
		if (var != null) {
			if (var.isConstant()) throw new IllegalStateException("Attempt to change the value of constant variable " + name);
			var.setValue(val);
			return var;
		}
		var = createVariable(name, val);
