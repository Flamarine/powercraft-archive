...
public final class PC_MSGRegistry {
	...
	public static final int MSG_DEFAULT_NAME = 1, MSG_BLOCK_FLAGS = 2,
			MSG_ITEM_FLAGS = 3, MSG_RENDER_INVENTORY_BLOCK = 4,
			MSG_RENDER_WORLD_BLOCK = 5, MSG_LOAD_FROM_CONFIG = 10,
			MSG_ON_HIT_BY_BEAM_TRACER = 11, MSG_BURN_TIME = 12,
			MSG_RECIVE_POWER = 13, MSG_CAN_RECIVE_POWER = 14,
			MSG_ON_ACTIVATOR_USED_ON_BLOCK = 15,
			MSG_STR_MSG = 17,
			MSG_RENDER_ITEM_HORIZONTAL = 18, MSG_ROTATION = 19,
			MSG_RATING = 20, MSG_CONDUCTIVITY = 21, MSG_LOAD_WORLD = 23, 
			MSG_GET_PROVIDET_GLOBAL_FUNCTIONS = 24, MSG_RENDER_OVERLAY = 25, 
			MSG_OPEN_GUI_OR_PLACE_BLOCK = 26, MSG_DOES_SMOKE = 27;
	...
	public static Object callBlockMSG(IBlockAccess world, PC_VecI pos,
			int msg, Object... o) {
		return callBlockMSG(world, pos.x, pos.y, pos.z, msg, o);
	}

	public static Object callBlockMSG(IBlockAccess world, int x, int y,
			int z, int msg, Object... o) {
	...
	public static Object callAllMSG(MSGIterator iterator, int msg,
			Object... o) {
		List<PC_IMSG> objs = getMSGObjects();
		for (PC_IMSG obj : objs) {
			Object ret = obj.msg(msg, o);
	...
	public static boolean hasFlag(World world, PC_VecI pos, String flag) {
		...
		if (b instanceof PC_IMSG) {
			List<String> list = (List<String>) ((PC_IMSG) b).msg(
					MSG_BLOCK_FLAGS, world, pos,
	...
	public static boolean hasFlag(ItemStack is, String flag) {
		...
		if (i instanceof PC_IMSG) {
			List<String> list = (List<String>) ((PC_IMSG) i).msg(
					MSG_ITEM_FLAGS, is, new ArrayList<String>());
			if (list != null) {
				return list.contains(flag);
			}
		}
