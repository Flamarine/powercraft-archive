...
public final class PC_DataHandlerRegistry {
	...
	public static void savePowerCraftData(WorldInfo worldInfo,
			File worldDirectory) {
		worldDirectory = new File(worldDirectory, "powercraft");
		if (!worldDirectory.exists())
			worldDirectory.mkdirs();
		for (Entry<String, PC_IDataHandler> dataHandler : dataHandlers
				.entrySet()) {
			if (dataHandler.getValue().needSave()) {
				NBTTagCompound nbttag = dataHandler.getValue().save(
						new NBTTagCompound(dataHandler.getKey()));
				try {
					File file = new File(worldDirectory,
							dataHandler.getKey() + ".dat");
	...
	public static void loadPowerCraftData(WorldInfo worldInfo,
			File worldDirectory) {
		worldDirectory = new File(worldDirectory, "powercraft");
